...
当前实机验证以 M4 Max MacBook Pro 为主。其他机型、macOS 版本和显示环境不应视为已验证，具体范围见 [0.2.3.18 验证记录](docs/macfanpro-0.2.3.18-validation.md)。
...
例如，`0.2.3.18` 解压在“下载”目录时：

```bash
cd ~/Downloads/MacFanPro-0.2.3.18-macos-arm64
...
此方式默认构建仓库的 `main` 分支，可能包含尚未发行的修改。如需构建指定发行版，可在运行 `./setup.sh` 前执行 `git checkout v0.2.3.18`，版本号按需替换。
