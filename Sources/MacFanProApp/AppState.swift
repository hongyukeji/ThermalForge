...
import SwiftUI
@preconcurrency import MacFanProCore
...
final class AppState: ObservableObject {
    ...
    /// until `sudo macfanpro install` re-syncs it.
    @Published var daemonVersionMismatch: String?
    /// A hold set from the CLI (`sudo macfanpro max`) that the app is
    ...
    private let heartbeatQueue = DispatchQueue(label: "io.github.macfanpro.heartbeat", qos: .utility)
    ...
    private func startHeartbeat() {
        ...
        timer.setEventHandler { [weak self] in
            ...
            do {
                ...
                versionValue = (daemonVersion == MacFanProVersion.current) ? nil : daemonVersion
                didReadVersion = true
    ...
    private static func storedAvailableUpdate() -> AvailableUpdate? {
        ...
        return UpdateChecker.evaluate(
            current: MacFanProVersion.current,
            tagName: version,
    ...
    func restartDaemon() {
        let label = MacFanProDaemon.label
