...
import SwiftUI
@preconcurrency import ThermalForgeProCore
...
final class AppState: ObservableObject {
    ...
    /// until `sudo thermalforgepro install` re-syncs it.
    @Published var daemonVersionMismatch: String?
    /// A hold set from the CLI (`sudo thermalforgepro max`) that the app is
    ...
    private let heartbeatQueue = DispatchQueue(label: "io.github.hongyukeji.thermalforgepro.heartbeat", qos: .utility)
    ...
    private func startHeartbeat() {
        ...
        timer.setEventHandler { [weak self] in
            ...
            do {
                ...
                versionValue = (daemonVersion == ThermalForgeProVersion.current) ? nil : daemonVersion
                didReadVersion = true
    ...
    private static func storedAvailableUpdate() -> AvailableUpdate? {
        ...
        return UpdateChecker.evaluate(
            current: ThermalForgeProVersion.current,
            tagName: version,
    ...
    func restartDaemon() {
        let label = ThermalForgeProDaemon.label
