...
struct MenuBarView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            // Header
            HStack {
                Text(language.text("ThermalForgePro"))
                    .font(.headline)
                Spacer()
                stateIndicator
            }
            ...
            .accessibilityIdentifier("io.github.hongyukeji.thermalforgepro.language")
            .padding(.horizontal, 12)
            .padding(.top, 4)
            .padding(.bottom, 6)
...
private struct DaemonUpdateBanner: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            Text(language.text("The background service is running {daemonVersion}, but the app is {appVersion}. Fan control may not match what you set until they're re-synced.", ["daemonVersion": daemonVersion, "appVersion": ThermalForgeProVersion.current]))
                .font(.caption)
                .foregroundStyle(.secondary)
                .fixedSize(horizontal: false, vertical: true)
            ...
            Text("sudo thermalforgepro install")
...
/// Shown when a newer ThermalForgePro release exists than the installed build. Purely
...
private struct UpdateAvailableBanner: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            Text(language.text("ThermalForgePro {version} is available. You have {appVersion}.", ["version": update.version, "appVersion": ThermalForgeProVersion.current]))
                .font(.caption)
                .foregroundStyle(.secondary)
                .fixedSize(horizontal: false, vertical: true)
            ...
            Text("brew upgrade thermalforgepro && sudo thermalforgepro install")
                .font(.system(.caption, design: .monospaced))
                .textSelection(.enabled)
                .fixedSize(horizontal: false, vertical: true)
                .padding(.horizontal, 6)
