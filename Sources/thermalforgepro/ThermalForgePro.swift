...
@main
struct ThermalForgePro: ParsableCommand {
    static let configuration = CommandConfiguration(
        commandName: "thermalforgepro",
        abstract: "Fan control for Apple Silicon MacBooks",
        version: ThermalForgeProVersion.current,
        subcommands: [
            Max.self,
            Auto.self,
            SetSpeed.self,
...
/// it keeps running the OLD binary until `thermalforgepro install` re-syncs it, so
...
func warnIfDaemonVersionMismatch() {
    guard ThermalForgeProDaemon.isRunning else { return }
    ...
    do {
        ...
        guard daemonVersion != ThermalForgeProVersion.current else { return }
    ...
    case .daemonHoldWillRevert(let version):
        message = """
            ...
            supervision — it will revert to auto in about 15 seconds.
                Re-sync to make holds stick:  sudo thermalforgepro install
    ...
    case .directOldDaemon(let version):
        message = """
            ...
            commands, so this used a direct hardware write (which needs sudo).
                Re-sync to drop the sudo requirement:  sudo thermalforgepro install
    ...
    case .directLegacyDaemon:
        message = """
            ...
            control protocol, so this used a direct hardware write.
                Finish the upgrade to reconnect it:  sudo thermalforgepro install
...
struct Auto: ParsableCommand {
    ...
    func run() throws {
        ...
        if stopApp {
            ...
            kill.arguments = ["ThermalForgeProApp"]
            try? kill.run()
            kill.waitUntilExit()
        }
...
struct Discover: ParsableCommand {
    ...
    func run() throws {
        ...
        var lines: [String] = []
        lines.append("ThermalForgePro Key Dump")
...
struct Watch: ParsableCommand {
    ...
    func run() throws {
        ...
        print("ThermalForgePro watch — profile: \(selectedProfile.name)")
...
struct Calibrate: ParsableCommand {
    ...
    func run() throws {
        ...
        guard geteuid() == 0 else {
            throw ValidationError("Run with sudo: sudo thermalforgepro calibrate")
        }
        ...
        print("ThermalForgePro Calibration")
        print("========================")
        ...
        print("This is within normal operating parameters but ThermalForgePro is provided")
...
struct Install: ParsableCommand {
    ...
    func run() throws {
        guard geteuid() == 0 else {
            throw ValidationError("Run with sudo: sudo thermalforgepro install")
        }
        ...
        let appCandidates = [
            sourceBinary.deletingLastPathComponent().appendingPathComponent("ThermalForgePro.app").path,
            sourceBinary.deletingLastPathComponent().deletingLastPathComponent().appendingPathComponent("ThermalForgePro.app").path,
            "/opt/homebrew/opt/thermalforgepro/ThermalForgePro.app",
            "/usr/local/opt/thermalforgepro/ThermalForgePro.app",
        ]
        guard appCandidates.contains(where: {
            let info = NSDictionary(contentsOfFile: "\($0)/Contents/Info.plist")
            return info?["CFBundleShortVersionString"] as? String == ThermalForgeProVersion.current
                && info?["CFBundleIdentifier"] as? String == "io.github.hongyukeji.thermalforgepro.app"
        }) else {
            throw ValidationError("No matching ThermalForgePro.app was found. Install with Homebrew or run ./setup.sh before installing the daemon.")
        }

        let migration: LegacyMigration?
        if LegacyMigration.isPresent {
            guard migrateThermalforge else {
                throw ValidationError("ThermalForge is installed. To replace it, run: sudo thermalforgepro install --migrate-thermalforge")
            }
        ...
        let prePid = runToolOutput("/usr/bin/pgrep", ["-x", "-u", "\(ownerUID)", "ThermalForgeProApp"])
        let appWasRunning = prePid != nil || migration?.appWasRunning == true

        // Resolve symlinks first. Launched via Homebrew (`sudo thermalforgepro
        // install`), argv[0] is /opt/homebrew/bin/thermalforgepro — itself a symlink
        ...
        let installPath = ThermalForgeProDaemon.installPath
        ...
        if resolvedBinary != resolvedInstall {
            ...
            let current = ThermalForgeProVersion.current
            let kegBinaries = [
                "/opt/homebrew/opt/thermalforgepro/bin/thermalforgepro",
                "/usr/local/opt/thermalforgepro/bin/thermalforgepro",
            ]
            ...
            func kegVersion(_ path: String) -> String? {
                ...
                let parts = real.components(separatedBy: "/Cellar/thermalforgepro/")
            ...
            for keg in kegBinaries {
                ...
                guard let version = kegVersion(keg) else {
                    ...
                    // "/Cellar/thermalforgepro/"). Say so loudly: otherwise re-sync goes
                    ...
                    print("""
                        ...
                        .../Cellar/thermalforgepro/<version>/...). Skipping re-sync — \
                        check whether the formula was renamed.
                        """)
                    continue
                }
                // Strictly newer than what's installed (numeric, never downgrade).
                guard ThermalForgeProVersion.atLeast(version, current),
                      !ThermalForgeProVersion.atLeast(current, version) else { continue }
        ...
        let plist = """
            ...
            <dict>
                <key>Label</key>
                <string>\(ThermalForgeProDaemon.label)</string>
            ...
            """
        try plist.write(
            toFile: ThermalForgeProDaemon.plistPath,
        ...
        unlink("/tmp/thermalforgepro.sock")

        // Start new daemon
        let load = Process()
        load.executableURL = URL(fileURLWithPath: "/bin/launchctl")
        load.arguments = ["bootstrap", "system", ThermalForgeProDaemon.plistPath]
        try load.run()
        load.waitUntilExit()

        // Verify
        Thread.sleep(forTimeInterval: 1.0)
        guard ThermalForgeProDaemon.isRunning else {
            throw ValidationError("""
                The background daemon didn't come up after install, so the menu bar
                app won't be able to control fans yet.

                What to do:
                  1. Run it again:  sudo thermalforgepro install
                  2. If it still fails, the copy at \(installPath) may not be
                     executable or is crashing on launch. Open Console.app, search
                     "io.github.hongyukeji.thermalforgepro.daemon", and check the most recent error.
                """)
        }
        ...
        //   1. Next to the running binary (<keg>/bin/thermalforgepro -> <keg>/ThermalForgePro.app):
        //      the normal `sudo thermalforgepro install` path, via Homebrew's bin symlink.
        ...
        let appDest = "/Applications/ThermalForgePro.app"
        ...
        let wantedVersion = ThermalForgeProVersion.current
        ...
        if let appSource = candidates.first(where: {
            ...
            print("Installed ThermalForgePro.app to \(appDest)")
        ...
        // fails the install.
        let moveGuidance = "Quit and reopen ThermalForgePro — the fan-control socket moved this version."
        if appWasRunning && freshBundleInstalled {
            _ = runTool("/usr/bin/pkill", ["-x", "-u", "\(ownerUID)", "ThermalForgeProApp"])
            ...
            for _ in 0..<5 {
                ...
                if let newPid = runToolOutput("/usr/bin/pgrep", ["-x", "-u", "\(ownerUID)", "ThermalForgeProApp"]),
                   newPid != prePid {
                    relaunched = true
                    break
                }
...
struct Uninstall: ParsableCommand {
    ...
    @Flag(name: .long, help: "Also delete this user's ThermalForgePro profiles, calibration and logs")
    var purgeData = false

    func run() throws {
        guard geteuid() == 0 else {
            throw ValidationError("Run with sudo: sudo thermalforgepro uninstall")
        }
        ...
        // Remove daemon files
        try? fm.removeItem(atPath: ThermalForgeProDaemon.plistPath)
        try? fm.removeItem(atPath: ThermalForgeProDaemon.installPath)
        try? fm.removeItem(atPath: ThermalForgeProDaemon.socketPath)      // /var/run (current)
        try? fm.removeItem(atPath: "/tmp/thermalforgepro.sock")           // legacy path (pre-Phase 1)

        // Remove user data
        let appSupport = home.appendingPathComponent("Library/Application Support/ThermalForgePro")
        let logs = home.appendingPathComponent("Library/Logs/ThermalForgePro")
        if purgeData {
        ...
        // Remove app bundle
        try? fm.removeItem(atPath: "/Applications/ThermalForgePro.app")

        print("ThermalForgePro fully uninstalled.")
        ...
        print("If installed with Homebrew, finish with: brew uninstall thermalforgepro")
    }
}

// MARK: - BuildApp (internal)

/// Assembles ThermalForgePro.app from a built app binary + icon, writing the
/// Info.plist from ThermalForgeProVersion.current. This is the SINGLE place the
...
struct BuildApp: ParsableCommand {
    static let configuration = CommandConfiguration(
        ...
        abstract: "Assemble ThermalForgePro.app from a built app binary and icon (internal)",
        shouldDisplay: false
    )

    @Option(name: .long, help: "Path to the built ThermalForgeProApp executable")
    var binary: String
    ...
    func run() throws {
        ...
        try fm.copyItem(atPath: binary, toPath: "\(macOSDir)/ThermalForgeProApp")
        ...
        let plist = """
            ...
            <dict>
                ...
                <string>ThermalForgePro</string>
                <key>CFBundleDisplayName</key>
                <string>ThermalForgePro</string>
                <key>CFBundleIdentifier</key>
                <string>io.github.hongyukeji.thermalforgepro.app</string>
                <key>CFBundleDevelopmentRegion</key>
                <string>en</string>
                <key>CFBundleLocalizations</key>
                <array><string>en</string><string>zh-Hans</string><string>zh-Hant</string></array>
                <key>CFBundleVersion</key>
                <string>\(ThermalForgeProVersion.current)</string>
                <key>CFBundleShortVersionString</key>
                <string>\(ThermalForgeProVersion.current)</string>
                <key>CFBundleExecutable</key>
                <string>ThermalForgeProApp</string>
                <key>CFBundleIconFile</key>
                <string>AppIcon</string>
                <key>CFBundlePackageType</key>
                <string>APPL</string>
                <key>LSMinimumSystemVersion</key>
                <string>\(ThermalForgeProVersion.minimumMacOS)</string>
                <key>LSUIElement</key>
                <true/>
        ...
        print("Assembled \(dest) (version \(ThermalForgeProVersion.current))")
    }
}
