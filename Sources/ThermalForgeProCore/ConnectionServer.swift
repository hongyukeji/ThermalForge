...
final class ConnectionServer: @unchecked Sendable {
    ...
    private let acceptQueue = DispatchQueue(label: "io.github.hongyukeji.thermalforgepro.accept")
    ...
    private func handleConnection(_ fd: Int32) {
        ...
        readExactly(io, length: 4, queue: q) { [self] header in
            ...
            case .legacyPeer:
                ...
                NSLog("ThermalForgePro daemon: legacy (pre-Phase-2) client — advising reinstall")
                writeRaw(io, "error: daemon protocol updated; reinstall the CLI: sudo thermalforgepro install\n",
                         queue: q, completion: finish)
            case .oversized:
                NSLog("ThermalForgePro daemon: rejected oversized request frame")
    ...
    private func writeResponse(_ io: DispatchIO, _ response: DaemonResponse, queue: DispatchQueue,
        ...
        guard let frame = try? DaemonProtocol.encodeFrame(response, max: DaemonProtocol.maxResponseBytes) else {
            NSLog("ThermalForgePro daemon: response exceeds frame cap; dropping connection")
            completion(); return
        }
