...
//  ThermalForgePro
//
//  Daily rotating app log to ~/Library/Logs/ThermalForgePro/
//  One file per day (thermalforgepro-2026-04-05.log).
...
public final class TFLogger {
    ...
    private var currentLogFile: URL {
        ...
        return logDir.appendingPathComponent("thermalforgepro-\(dateStr).log")
    }
    ...
    private func cleanExpiredLogs() {
        ...
        for file in files {
            ...
            // Match thermalforgepro-YYYY-MM-DD.log pattern
            guard name.hasPrefix("thermalforgepro-") && name.hasSuffix(".log") else { continue }
            let dateStr = String(name.dropFirst("thermalforgepro-".count).dropLast(".log".count))
        ...
        let oldLog = logDir.appendingPathComponent("thermalforgepro.log")
