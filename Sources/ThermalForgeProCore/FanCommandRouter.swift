    ...
    public var description: String {
        ...
        case .rootRequired:
            return """
                ...
                Run it with sudo, or install the background daemon:  sudo thermalforgepro install
                """
        case .perFanNeedsNewerDaemon(let version):
            return """
                ...
                Run it with sudo, or re-sync the daemon:  sudo thermalforgepro install
                """
        case .legacyDaemonNeedsReinstall:
            return """
...
public enum FanCommandRouter {
    ...
    public static func apply(_ command: FanCommand, oneshot: Bool) throws -> (route: FanRoute, note: String?, appliedRPM: Int?) {
        ...
        let protocolKnownUnsupported = daemonVersion.map {
            !ThermalForgeProVersion.atLeast($0, ThermalForgeProVersion.oneshotProtocolSince)
        } ?? false
