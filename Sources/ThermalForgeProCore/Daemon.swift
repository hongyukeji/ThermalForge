...
public enum ThermalForgeProDaemon {
    ...
    public static let socketPath = "/var/run/thermalforgepro.sock"
    public static let plistPath = "/Library/LaunchDaemons/io.github.hongyukeji.thermalforgepro.daemon.plist"
    public static let installPath = "/usr/local/bin/thermalforgepro"
    public static let label = "io.github.hongyukeji.thermalforgepro.daemon"
    ...
    public static var isRunning: Bool {
        ...
        // reset path: `sudo thermalforgepro auto` → FanCommandRouter.apply → this
    ...
    public static func bootoutIfRegistered() throws {
        ...
        guard p.terminationStatus == 0 else {
            throw ThermalForgeProError.writeFailed(
...
public enum DaemonError: Error, CustomStringConvertible {
    ...
    /// `brew upgrade` done, `sudo thermalforgepro install` not yet). Its socket is up so
    ...
    public var description: String {
        switch self {
        case .notRunning:
            return "ThermalForgePro daemon is not running. Run: sudo thermalforgepro install"
        case .connectionFailed:
            return "Failed to connect to daemon socket"
        case .commandFailed(let msg):
            return "Daemon error: \(msg)"
        case .incompatibleDaemon:
            return "The background daemon is an older build using the previous control protocol. Reinstall to reconnect: sudo thermalforgepro install"
        }
    }
}
...
public final class DaemonClient {
    ...
    public func request(_ req: DaemonRequest, timeout: TimeInterval = 2.0) throws -> DaemonResponse {
        ...
        var addr = sockaddr_un()
        addr.sun_family = sa_family_t(AF_UNIX)
        setPath(&addr, ThermalForgeProDaemon.socketPath)
...
/// watchdog-reverted — a fire-and-forget `thermalforgepro max`) from a supervised
...
public final class DaemonServer {
    ...
    /// uid of the controlling user (who ran `sudo thermalforgepro install`). The
    ...
    public init(fanControl: FanControl, ownerUID: uid_t,
        ...
        guard ownerUID != 0 else {
            NSLog("ThermalForgePro daemon: refusing to start — owner uid is 0. Reinstall with `sudo thermalforgepro install` from your user account.")
            throw ThermalForgeProError.writeFailed("daemon owner uid must be non-zero")
        }
        ...
        guard bindResult == 0 else {
            close(fd)
            throw ThermalForgeProError.writeFailed("bind() failed: \(errno)")
        }
        ...
        guard chown(ThermalForgeProDaemon.socketPath, ownerUID, 0) == 0 else {
            let err = errno
            NSLog("ThermalForgePro daemon: chown of the socket to uid %u failed: errno %d", ownerUID, err)
            close(fd)
            throw ThermalForgeProError.writeFailed("chown() failed: errno \(err)")
        }
        guard chmod(ThermalForgeProDaemon.socketPath, 0o600) == 0 else {
            let err = errno
            NSLog("ThermalForgePro daemon: chmod(0600) on the socket failed: errno %d", err)
            close(fd)
            throw ThermalForgeProError.writeFailed("chmod() failed: errno \(err)")
        }

        guard listen(fd, 16) == 0 else {   // backlog holds queued connects while at the 8-handler cap
            close(fd)
            throw ThermalForgeProError.writeFailed("listen() failed")
        }
    }

    /// Run the server loop (blocks forever)
    public func run() {
        NSLog("ThermalForgePro daemon: listening on %@", ThermalForgeProDaemon.socketPath)
        // Start log maintenance even when no fan command has been issued.
        TFLogger.shared.daemon("Listening on \(ThermalForgeProDaemon.socketPath)")
        ...
        DispatchQueue.global(qos: .utility).async { [self] in
            while true {
                ...
                if suspended {
                    ...
                    stateLock.unlock()
                    NSLog("ThermalForgePro daemon: supervised hold timed out during thermal suspension — cleared; fans stay at max until cooldown")
                    continue
                }
                ...
                autoreleasepool {
                    NSLog("ThermalForgePro daemon: heartbeat timeout — resetting fans to auto")
                    smcLock.lock()
                    ...
                    do {
                        ...
                        NSLog("ThermalForgePro daemon: watchdog reset failed: %@, will retry", "\(error)")
    ...
    private func thermalTick() {
        ...
        // it could not act on is not safety. The floor exists for one case: ThermalForgePro
        ...
        case .engage:
            ...
            NSLog("ThermalForgePro daemon: thermal floor engaged at %.1f°C — fans held at max (was %@)",
        ...
        case .restore:
            ...
            NSLog("ThermalForgePro daemon: thermal floor cleared at %.1f°C — %@",
    ...
    private func registerWakeNotification() {
        ...
        guard rootPort != 0, let notifyPort = notifyPort else {
            NSLog("ThermalForgePro daemon: failed to register for power notifications")
            return
        }

        let source = IONotificationPortGetRunLoopSource(notifyPort).takeUnretainedValue()
        CFRunLoopAddSource(CFRunLoopGetMain(), source, .defaultMode)
        NSLog("ThermalForgePro daemon: registered for wake notifications")
    }
    ...
    private func handleWake() {
        ...
        guard let command = heldCommand else {
            NSLog("ThermalForgePro daemon: woke — no profile to re-apply")
            return
        }

        NSLog("ThermalForgePro daemon: woke — re-applying: %@", command)
        ...
        DispatchQueue.global(qos: .utility).asyncAfter(deadline: .now() + 2.0) { [self] in
            ...
            do {
                ...
                NSLog("ThermalForgePro daemon: re-applied after wake")
            } catch {
                NSLog("ThermalForgePro daemon: wake re-apply failed: %@", "\(error)")
            }
        }
    }
    ...
    private func processFrame(_ body: Data) -> DaemonResponse {
        guard let request = try? DaemonProtocol.decode(DaemonRequest.self, from: body) else {
            ...
            return .unsupported(daemonVersion: ThermalForgeProVersion.current)
        }
        ...
        NSLog("ThermalForgePro daemon: verb=%@ outcome=%@", request.verb.rawValue,
...
/// `DaemonClient.sendRaw` (throws on false) and `ThermalForgeProDaemon.isRunning`
