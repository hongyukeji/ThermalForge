...
public final class TFLogger {
    public static let shared = TFLogger(
        directory: FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent("Library/Logs/MacFanPro"),
    ...
    private let queue = DispatchQueue(label: "MacFanPro.runtime-log", qos: .utility)
...
final class RuntimeLogStore {
    ...
    static func name(for date: Date) -> String { "macfanpro-\(dateFormatter().string(from: date)).log" }
    ...
    private func files() throws -> [File] {
        ...
        return try fm.contentsOfDirectory(at: directory, includingPropertiesForKeys: [.isRegularFileKey, .isSymbolicLinkKey, .fileSizeKey]).compactMap { url in
            ...
            if name == "macfanpro.log" { return File(url: url, day: .distantPast, index: 0, bytes: values.fileSize ?? 0) }
            guard name.hasPrefix("macfanpro-") else { return nil }
            let parts = name.dropFirst("macfanpro-".count).split(separator: ".", omittingEmptySubsequences: false)
